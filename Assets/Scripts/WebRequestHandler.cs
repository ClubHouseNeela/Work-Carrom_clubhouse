...
public class WebRequestHandler : MonoBehaviour
{
    ...
    private IEnumerator GetRequest(string url, Action<string, bool> OnRequestProcessed)
    {
        ...
        request.SetRequestHeader("token", AndroidtoUnityJSON.instance.token);
        yield return request.SendWebRequest();
        if (request.result == UnityWebRequest.Result.ConnectionError || request.result == UnityWebRequest.Result.ProtocolError)
        {
            Debug.LogError(url + " web request error in Get method with responce code : " + request.responseCode);
    ...
    {
        Debug.Log("Url:  " + url + " json request: " + json);
        var request = new UnityWebRequest(url, "POST");
        request.SetRequestHeader("token", AndroidtoUnityJSON.instance.token); //set this according to the android json data
        ...
        {
            Debug.Log("Url:  " + url + " json response: " + request.downloadHandler.text);
            OnRequestProcessed(request.downloadHandler.text, true);
        }
